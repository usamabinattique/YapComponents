...
public extension UIViewController {
    ...
    func showActionSheet(title: String? = nil, message: String? = nil, defaultButtonTitle: String = "OK", style: UIAlertController.Style = .actionSheet, defaultButtonHandler: ((UIAlertAction) -> Void)? = nil, actions: UIAlertAction..., completion: (() -> Void)? = nil) {
...
extension UIViewController {
    ...
    public func addBackButton(_ type: BackButtonType = .backCircled, backgroundColor: UIColor = .primary, tintColor: UIColor = .white) {
        
        let button = UIButton()
        button.frame = CGRect(x: 0, y: 0, width: 35, height: 35)
        button.setImage(UIImage(named: type.icon, in: yapKitBundle, compatibleWith: nil)?.asTemplate, for: .normal)
        button.tintColor = type == .backCircled || type == .closeCircled ? .white : .primary
    ...
    public func addBackButtonWithOutNavBar(_ type: BackButtonType = .backCircled) {
        ...
        let button = UIButton()
        button.frame = CGRect(x: 20, y: 44, width: 35, height: 35)
        button.setImage(UIImage(named: type == .backCircled || type == .backEmpty ? "icon_back" : "icon_close", in: yapKitBundle, compatibleWith: nil)?.asTemplate, for: .normal)
        button.tintColor = type == .backCircled || type == .closeCircled ? .white : .primary
