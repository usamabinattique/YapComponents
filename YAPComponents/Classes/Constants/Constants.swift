...
public extension TransactionProductCode {
    ...
    func icon(forTransactionType transactionType: TransactionType?, transactionStatus: TransactionStatus) -> UIImage? {
        ...
        guard transactionStatus == .completed || transactionStatus == .inProgress || transactionStatus == .pending || transactionStatus == .cancelled || transactionStatus == .failed else {
            return UIImage.init(named: "icon_delayed_transfer", in: yapKitBundle, compatibleWith: nil)?.asTemplate
        }
        
        guard transactionStatus != .cancelled && transactionStatus != .failed  else {
            return UIImage.init(named: "icon_cancelled_transaction", in: yapKitBundle, compatibleWith: nil)
        }
        
        guard !isBank else { return UIImage.init(named: "icon_send_money", in: yapKitBundle, compatibleWith: nil)?.asTemplate }
        
        guard !isCash else { return UIImage.init(named: "icon_cash_payout", in: yapKitBundle, compatibleWith: nil)?.asTemplate }
        
        guard !isRefund else { return UIImage.init(named: "icon_refund_transaction", in: yapKitBundle, compatibleWith: nil)?.asTemplate }
        
        guard !isFee else { return UIImage.init(named: "icon_transaction_fee", in: yapKitBundle, compatibleWith: nil) }
        
        guard !isIncoming else { return UIImage.init(named: "icon_incoming_transaction", in: yapKitBundle, compatibleWith: nil) }
        
        switch self {
            
        case .y2yTransfer:
            return UIImage.init(named: "icon_y2y_transfer", in: yapKitBundle, compatibleWith: nil)?.asTemplate
