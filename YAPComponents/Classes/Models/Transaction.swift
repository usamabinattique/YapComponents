...
extension Transaction {
    public var prepaidCardName: String? {
        return cardType == "PREPAID" ? cardName1 : cardName2
    }
    
    public var finalizedTitle: String? {
        switch productCode {
        ...
        case .posPurchase, .eCom:
            return merchantName.map { "Spent at \($0)" }
        case .atmDeposit:
            return "Cash deposit"
        case .atmWithdrawl:
            return "Withdrawal"
    ...
    public var unwrappedStatus: TransactionStatus { TransactionStatus.init(rawValue: status ?? "") ?? .none }
    ...
    public var detailTransferType: String {
