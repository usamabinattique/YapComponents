...
public class ResourceManager {
    ...
    public class var appCountries: [AppCountry]? {
        guard let url = yapKitBundle.url(forResource: "Countries", withExtension: "json"),
...
var ibanValidationRegexes: [String: String] {
    if let path = yapKitBundle.path(forResource: "IBANRegex", ofType: "plist") {
...
public func appShareMessageForY2Y(welcomeMessage message: String, appShareUrl: String) -> String {
    return String.init(format: "common_display_text_y2y_share".localized, message, appShareUrl)
}

public func appShareMessageForMore( _ downloadURL: String) -> String {
    return String.init(format: "screen_invite_friend_display_text_share_url".localized, downloadURL)
}

public func appInviteWaitingList( _ downloadURL: String) -> String {
    return String.init(format: "screen_waiting_list_rank_share_invite_text".localized, downloadURL)
}

public func appShareMessageForCardsPlan( _ downloadURL: String) -> String {
    return String.init(format: "screen_invite_friend_cards_&_plan_display_text_share_url".localized, downloadURL)
}
