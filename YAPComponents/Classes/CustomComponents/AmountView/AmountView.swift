...
open class AmountView: UIView {
    public lazy var headingLabel: UILabel = UILabelFactory.createUILabel(with: .greyDark, textStyle: .micro, alignment: .center)
    ...
    func bindAmountFieldFormatting() {
        ...
        Observable
            ...
            .map { (params: (Double?, String?)) -> String? in
                params.1 != nil ? params.0.map { floor($0) }.map { $0.withGroupingSeparator() + params.1! } : params.0.map { $0.withGroupingSeparator() } }
