...
public class PopoverView: UIView {
    ...
    private lazy var dateLabel: UILabel = UILabelFactory.createUILabel(with: .greyDark, textStyle: .micro, alignment: .center)
    
    private lazy var amountLabel: UILabel = UILabelFactory.createUILabel(with: .primaryDark, textStyle: .micro, alignment: .center)
    
    private lazy var close: UIButton = UIButtonFactory.createButton()
    
    private lazy var closeImage = UIImageViewFactory.createImageView(mode: .scaleAspectFit, image: UIImage.sharedImage(named: "icon_close")?.asTemplate, tintColor: .primaryDark)
