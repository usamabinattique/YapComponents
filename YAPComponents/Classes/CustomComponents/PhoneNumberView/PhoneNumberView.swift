...
//  YAPKit
//
//  Created by Wajahat Hassan on 28/09/2019.
...
open class PhoneNumberView: UIView {
    ...
    public lazy var textField: UITextField = {
        ...
        textField.textColor = UIColor.primaryDark.withAlphaComponent(0.5)
    ...
    public var validationState: PhoneNumberView .ValidationState = .normal {
        didSet {
            ...
            bottomBar.backgroundColor = validationState == .invalid ? .error : .primary
...
extension PhoneNumberView: UITextFieldDelegate {
    ...
    public func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        title.textColor = (textField.text == nil || textField.text?.count ?? 0 == 0) ? .primaryDark : .greyDark
...
private extension PhoneNumberView {
    func animateFocus() {
        UIView.animate(withDuration: 0.3) { [unowned self] in
            self.title.textColor = .greyDark
        }
    }
    
    func deanimateFocus() {
        UIView.animate(withDuration: 0.3, animations: { [unowned self] in
            self.title.textColor = .primaryDark
        })
    }
}
