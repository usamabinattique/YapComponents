...
public class CodeVerificationTextField: UITextField {
    ...
    @objc func textChanged() {
        _ = textLabels.map { [unowned self] in
            $0.highlight = false
            guard let index = self.textLabels.firstIndex(of: $0), text?.count ?? 0 > index else {
                $0.text = nil
                return
            }
            $0.text = text?[index]
...
private class BorderedLabel: UILabel {
    ...
    private func commonInit() {
        ...
        textColor = UIColor.appColor(ofType: .primaryDark)
        backgroundColor = UIColor.appColor(ofType: .greyLight).withAlphaComponent(0.36)
    ...
    var highlight: Bool = false {
        didSet {
            layer.borderColor = highlight ? UIColor.appColor(ofType: .primary).cgColor : UIColor.clear.cgColor
        }
    }
}
