...
open class ListViewController: UIViewController {
    ...
    private lazy var titleLabel: UILabel = UILabelFactory.createUILabel(with: .primaryDark, textStyle: .large, alignment: .center, numberOfLines: 0, lineBreakMode: .byWordWrapping)
...
private extension ListViewController {
    ...
    func __completeShow(_ velocity: CGFloat = 0) {
        ...
        UIView.animate(withDuration: time, animations: { [weak self] in
            self?.listContainer.frame.origin.y = (self?.view.bounds.height ?? UIScreen.main.bounds.height) - (self?.listContainer.bounds.height ?? 0)
